...
public interface Controller<
	...
	@GetMapping
	default ResponseEntity<?> getAll(@RequestParam(required = false) entity params,Pageable page) {
		Specification spec = new GSpecification<entity>().setSpecs(params);
		return ResponseEntity.status(HttpStatus.OK).body((PageImpl<?>) getService().list(params,spec,page));
	}
